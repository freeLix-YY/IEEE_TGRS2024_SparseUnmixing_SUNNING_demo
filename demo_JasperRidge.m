...
clearvars nBand nRow nCol Region SlectBands maxValue

p = 4; % p: number of endmembers
...
load("JasperRidge_GT.mat"); % M: GT library, XT: 2D abundance maps
...
load("spectral_library_jasperRidge.mat"); % spectra library: A

n = size(A,2); % n: total number of materials
nLib1 = size(lib1,2); % number of signatures in endmember 1 (Tree) library
nLib2 = size(lib2,2); % number of signatures in endmember 2 (Water) library
nLib3 = size(lib3,2); % number of signatures in endmember 3 (Dirt) library
nLib4 = size(lib4,2); % number of signatures in endmember 4 (Road) library

clearvars lib1 lib2 lib3 lib4 material_names

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Load noise case
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

load("JasperRidge_10%_Uniform_Noise.mat");
...
[LRUnSAL_TV.X_hat,~,~] = LRUnSAL_TV(A, Y, 'MU', 0.05, 'POSITIVITY', 'yes', 'ADDONE', 'no', ...
                                   'LAMBDA_1', 0.0001, 'LAMBDA_TV', 0.0001, 'LAMBDA_2', 0.001, 'TV_TYPE', 'niso',...
...
input.alpha = 50; % step-size
input.hyper_a = 100; % log-cosh
input.sparsity = 15; % l0-norm, even S > p, it eventually reduces to S = K
...
info.A_sub_index = [1, nLib1;
                    ...
                    nLib1+nLib2+nLib3+1, nLib1+nLib2+nLib3+nLib4]; % index of sub-class of A
