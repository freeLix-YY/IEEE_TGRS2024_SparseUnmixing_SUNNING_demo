...
clearvars nBand nRow nCol SlectBands maxValue

p = 6; % p: number of endmembers
...
load("Urban_GT.mat"); % M: GT library, XT: 2D abundance maps
...
load("spectral_library_urban.mat"); % spectra library: A

n = size(A,2); % n: total number of materials
nLib1 = size(lib1,2); % number of signatures in endmember 1 (Asphalt) library
nLib2 = size(lib2,2); % number of signatures in endmember 2 (Grass) library
nLib3 = size(lib3,2); % number of signatures in endmember 3 (Tree) library
nLib4 = size(lib4,2); % number of signatures in endmember 4 (Roof) library
nLib5 = size(lib5,2); % number of signatures in endmember 5 (Metal) library
nLib6 = size(lib6,2); % number of signatures in endmember 6 (Dirt) library

clearvars lib1 lib2 lib3 lib4 lib5 lib6 material_names

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Load noise case
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

load("Urban_10%_Uniform_Noise.mat");
...
LRUnSAL_TV.X_avg = [mean(LRUnSAL_TV.X_hat(1:nLib1,:),1);...
                    ...
                    mean(LRUnSAL_TV.X_hat((nLib1+nLib2+nLib3+1):(nLib1+nLib2+nLib3+nLib4),:),1);...
                    mean(LRUnSAL_TV.X_hat((nLib1+nLib2+nLib3+nLib4+1):(nLib1+nLib2+nLib3+nLib4+nLib5),:),1); ...
                    mean(LRUnSAL_TV.X_hat((nLib1+nLib2+nLib3+nLib4+nLib5+1):(nLib1+nLib2+nLib3+nLib4+nLib5+nLib6),:),1)];
...
input.alpha = 25; % step-size
input.hyper_a = 100; % log-cosh
input.sparsity = 20; % l0-norm, even S > p, it eventually reduces to S = K
...
info.A_sub_index = [1, nLib1;
                    nLib1+1, nLib1+nLib2;
                    nLib1+nLib2+1, nLib1+nLib2+nLib3;
                    nLib1+nLib2+nLib3+1, nLib1+nLib2+nLib3+nLib4;
                    nLib1+nLib2+nLib3+nLib4+1, nLib1+nLib2+nLib3+nLib4+nLib5;
                    nLib1+nLib2+nLib3+nLib4+nLib5+1, nLib1+nLib2+nLib3+nLib4+nLib5+nLib6]; % index of sub-class of A
...
SUNNING.X_avg = [mean(SUNNING.X_hat(1:nLib1,:),1);...
                 ...
                 mean(SUNNING.X_hat((nLib1+nLib2+nLib3+1):(nLib1+nLib2+nLib3+nLib4),:),1);...
                 mean(SUNNING.X_hat((nLib1+nLib2+nLib3+nLib4+1):(nLib1+nLib2+nLib3+nLib4+nLib5),:),1); ...
                 mean(SUNNING.X_hat((nLib1+nLib2+nLib3+nLib4+nLib5+1):(nLib1+nLib2+nLib3+nLib4+nLib5+nLib6),:),1)];
